...
describe('objectStoreTest', function () {
    ...
    it('V9testbindAssetStore002', 0, function (done) {
        ...
        let g_object = distributedObject.create(context, {name: "Sam", age: 20, isVis: false});
        expect(g_object == undefined).assertEqual(false);
     ...
     * @tc.type: FUNC
     */
    it('V9testsetSessionId003', 0, function (done) {
        console.log(TAG + "************* V9testsetSessionId003 start *************");
        let g_object = distributedObject.create(context, {name: "Sam", age: 20, isVis: false});
        expect(g_object == undefined).assertEqual(false);
        ...
        try {
            g_object.bindAssetStore("asset003", bindInfo, (err, data)=>{
                console.info("yltest run in callback err: "+err +" data common: "+data);
                expect(null).assertFail();
                done();
            });
        } catch (error) {
            console.info(`V9testsetSessionId003 err is: ${error.code} and msg is: ${error.message}`);
            expect(error.code == 401).assertTrue();
            done();
        }
    })    
